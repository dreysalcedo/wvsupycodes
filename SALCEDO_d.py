# -*- coding: utf-8 -*-
"""
Created on Sun Nov  1 16:52:35 2020
...
#=======================================================================================================================
network_type_1 = 4 * 1 * 3
network_type_2 = 4 * 10 * 3
network_type_3 = 4 * 2 * 2 * 3

network_type = network_type_3

from sklearn.neural_network import MLPClassifier
quickprop = MLPClassifier(hidden_layer_sizes=(network_type),solver='lbfgs',learning_rate_init=0.1,max_iter=600)

quickprop.fit(X_train, y_train)

X_train_array = X_train.values
y_train_array = y_train.values

from sklearn.model_selection import cross_val_score
scores = cross_val_score(quickprop, X_train_array,y_train_array,cv =3, scoring = 'accuracy')

#the samples are balanced across target classes hence the accuracy and the F1-score are almost equal.
f1_scores = cross_val_score(quickprop, X_train_array,y_train_array,cv =3, scoring ='f1_macro')
...
quickprop = MLPClassifier(hidden_layer_sizes=(network_type),solver='lbfgs',learning_rate_init=0.1,max_iter=100)
from sklearn.model_selection import ShuffleSplit
# Cross validation with 100 iterations to get smoother mean test and train
# score curves, each time with 20% data randomly selected as a validation set.
cv = ShuffleSplit(n_splits=100, test_size=0.2, random_state=0)
train_sizes, train_scores, validation_scores = learning_curve(
                                                   quickprop, X,  y, cv = cv, n_jobs=5)   
...
# Create plot
plt.title("Learning Curve Quickprop Method (4-2-2-3 Network)")
